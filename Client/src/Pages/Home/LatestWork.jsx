...
const LatestWork = () => {
    ...
    useEffect(() => {
        axios
            .get("http://localhost:5000/latestwork")
    ...
    return (
        <div>
            ...
            <MotionDiv className='p-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10 lg:gap-6 text-center justify-items-center'>
                {latesWork.map((work) => (
                    <Link to={`/latestwork/${work._id}`}>
                        <MotionDiv key={work.id} className="card bg-base-100 dark:bg-gray-800 w-full md:w-72 lg:w-96 shadow-lg">
                            <figure>
                                <img
                                    src={work.image}
                                    alt={work.title} />
